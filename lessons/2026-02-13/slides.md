---
theme: default
background: https://source.unsplash.com/1920x1080/?missionary,light,path
class: text-center
highlighter: shiki
lineNumbers: false
info: |
  ## Seminary Lesson - February 13, 2026
  Weekly lesson for Seminary class
  Lesson 200: The Importance of Obedience in Bringing Others to Christ
  Source materials available in ./materials/
drawings:
  persist: false
transition: slide-left
title: Seminary Lesson - February 13, 2026
mdc: true
---

# Seminary Lesson
## February 13, 2026

### Lesson 200: Missionary Preparation
**The Importance of Obedience in Bringing Others to Christ**

<div class="pt-12">
  <span @click="$slidev.nav.next" class="px-2 py-1 rounded cursor-pointer" hover="bg-white bg-opacity-10">
    Press Space for next page <carbon:arrow-right class="inline"/>
  </span>
</div>

<div class="abs-br m-6 flex gap-2">
  <button @click="$slidev.nav.openInEditor()" title="Open in Editor" class="text-xl slidev-icon-btn opacity-50 !border-none !hover:text-white">
    <carbon:edit />
  </button>
</div>

<!--
Presenter Notes:
- Welcome students warmly
- Today we're studying the importance of obedience in missionary work
- Main theme: Obedience qualifies us to have the Spirit and represent the Savior
- Lesson Purpose: To help you recognize the importance of obedience in doing the Savior's work as a missionary
- Class runs from 12:00-12:40 (40 minutes total)
-->

---
layout: default
---

# Opening Activities

<div class="space-y-6">

## 🙏 Opening Prayer

<div class="bg-blue-50 p-4 rounded-lg">

Ask someone to say the opening prayer

</div>

## 💬 QT Time with Annalisa

<div class="bg-purple-50 p-4 rounded-lg">

**Question:** Why did President Oaks choose these 4 areas to focus on in his first message to the youth?

<div class="mt-3 grid grid-cols-2 gap-2 text-sm">

<div class="p-2 bg-white/50 rounded">1. Strengthen Our Faith in Him</div>
<div class="p-2 bg-white/50 rounded">2. Increase Our Humility</div>
<div class="p-2 bg-white/50 rounded">3. Seek Help from Others</div>
<div class="p-2 bg-white/50 rounded">4. Be Patient</div>

</div>

</div>

</div>

<!--
Presenter Notes:
- Ask someone to say opening prayer (2 minutes)
- Annalisa leads QT Time discussion on President Oaks' message (5-8 minutes)
- She'll discuss why he chose these 4 specific areas for youth
- Follow-up questions: Which is most important? How are they connected?
- If Annalisa forgot, use backup question from materials/qt-time/backup-discussion.md
- Keep QT time to about 5-8 minutes to stay on schedule
-->

---
layout: section
---

# The Lighthouse Story

<div class="text-center space-y-4">

## A Choice to Make

<div class="text-xl text-gray-700">
Like a ship captain approaching a lighthouse, we all have choices about obedience
</div>

</div>

<!--
Presenter Notes:
- Share the lighthouse story from the lesson
- Battleship captain sees a light ahead and radios: "Adjust your heading 20 degrees"
- Reply: "Negative. Recommend you adjust your course 20 degrees"
- Captain: "This is a battleship. Move now!"
- Reply: "This is a lighthouse. Your choice."
- Point: We can insist on our own course, or choose to obey the Lord
- God's standards are like a lighthouse - fixed and safe
-->

---
layout: default
---

# Why Obedience Matters

<div class="space-y-4">

<div class="p-6 bg-gradient-to-r from-blue-50 to-teal-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"Personal righteousness is one of the most important qualifications to represent the Savior before the world. It also qualifies us to have the companionship of the Spirit of the Lord with us."

</div>

<div class="text-sm text-gray-600 mt-3">

— Elder Ulisses Soares, Quorum of the Twelve Apostles

</div>

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center">

<div class="text-base font-semibold">

Without obedience, we cannot have the Spirit. Without the Spirit, we cannot effectively share the gospel.

</div>

</div>

</div>

<!--
Presenter Notes:
- Elder Soares teaches that personal righteousness qualifies us to represent Christ
- It also qualifies us for the Spirit's companionship
- This applies to all members sharing the gospel, not just full-time missionaries
- If we're not obedient, we won't have the Spirit with us
- The Spirit is essential for missionary work
-->

---
layout: section
---

# Two Types of Standards

<div class="text-center space-y-4">

## God's Commandments & Missionary Standards

<div class="text-xl text-gray-700">
Some standards apply to everyone. Others are specific to full-time missionaries.
</div>

</div>

<!--
Presenter Notes:
- We'll look at both types of standards today
- God's commandments: for all of God's children
- Missionary standards: for full-time missionaries specifically
- Both are important for doing the Lord's work
-->

---
layout: default
---

# God's Commandments

<div class="space-y-3">

## Standards for All God's Children

<div class="p-4 bg-blue-50 rounded-lg text-sm">

These apply to all members of the Church, whether on a mission or not:

</div>

<div class="grid grid-cols-2 gap-3 text-sm">

<div class="p-3 bg-green-50 rounded-lg">

- **Law of Chastity**
- **Word of Wisdom**
- **Sabbath Day Observance**

</div>

<div class="p-3 bg-purple-50 rounded-lg">

- **Honesty**
- **Tithing**
- **Love and Service**

</div>

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center mt-3">

These commandments prepare us to share the gospel with power

</div>

</div>

<!--
Presenter Notes:
- These are eternal commandments from God
- They apply to all of us, all the time
- Living these standards qualifies us for the Spirit
- When we're obedient, we can invite others to come unto Christ
- Our example of obedience teaches others
-->

---
layout: default
---

# Missionary Standards

<div class="space-y-2">

## Specific to Full-Time Missionaries

<div class="p-3 bg-blue-50 rounded-lg text-sm">

These focus on physical and spiritual safety:

</div>

<div class="space-y-1.5 text-xs">

<div class="p-2.5 bg-green-50 rounded-lg">

**Always stay with your companion** - for safety and spiritual protection

</div>

<div class="p-2.5 bg-purple-50 rounded-lg">

**Avoid dangerous situations** - no power tools, heavy machinery, tractors, trailers, or truck beds

</div>

<div class="p-2.5 bg-yellow-50 rounded-lg">

**Media restrictions** - avoid TV, movies, video games, and unauthorized videos

</div>

<div class="p-2.5 bg-pink-50 rounded-lg">

**Physical contact** - limit to handshakes with opposite gender

</div>

</div>

</div>

<!--
Presenter Notes:
- These standards protect missionaries physically and spiritually
- They help missionaries stay focused on their purpose
- They maintain the dignity of the calling
- Even if we don't understand why, obedience brings blessings
- Some missionaries may feel these don't apply to them - but they do!
-->

---
layout: default
---

# Video: Stay Within the Lines

<div class="space-y-4">

<div class="p-6 bg-blue-50 rounded-lg">

**Watch:** "Stay Within the Lines" (5:10)

**Look for:** The importance of obedience to God's laws in missionary work

</div>

<div class="p-4 bg-purple-50 rounded-lg text-center">

https://www.churchofjesuschrist.org/media/video/2012-07-0007-stay-within-the-lines?lang=eng

</div>

</div>

<!--
Presenter Notes:
- Play the video "Stay Within the Lines" (5:10 minutes)
- This video illustrates why missionary standards matter
- Shows how obedience brings safety and power
- After video, ask: What stood out to you about obedience?
- Allow 1-2 students to briefly share (1 minute total)
- Total time for this slide: 6-7 minutes
-->

---
layout: default
---

# Personal Reflection

<div class="space-y-4">

<div class="p-8 bg-gradient-to-br from-yellow-50 to-pink-50 rounded-xl">

<div class="text-lg font-semibold mb-4">

Take a moment to prayerfully consider:

</div>

<div class="text-left space-y-2 text-base">

- Your current efforts to be obedient to Heavenly Father and Jesus Christ

- Things you are doing well

- Things you may need to adjust to better show your love to Them

</div>

<div class="text-sm italic text-gray-600 mt-4">

Invite the Holy Ghost to help you recognize areas for improvement

</div>

</div>

</div>

<!--
Presenter Notes:
- Give students 2 minutes for silent reflection
- This is personal - they don't need to share
- Encourage them to listen to the Spirit
- The goal is personal self-assessment
- This prepares them for the scripture study
-->

---
layout: section
---

# Scripture Study

<div class="text-center space-y-4">

## What Do the Scriptures Teach About Obedience?

<div class="text-xl text-gray-700">
Preparing a talk on obedience in missionary work
</div>

</div>

<!--
Presenter Notes:
- Now we'll study scriptures about obedience
- Students will prepare a 3-5 minute talk outline
- This helps them internalize the doctrine
- They'll tag scriptures as "obedience" in their apps
-->

---
layout: default
---

# Study These Scriptures

<div class="space-y-2">

## Tag Your Favorites as "obedience"

<div class="grid grid-cols-2 gap-2 text-xs">

<div class="p-2 bg-blue-50 rounded-lg leading-tight">

**[Deuteronomy 6:1–3](https://www.churchofjesuschrist.org/study/scriptures/ot/deut/6?lang=eng&id=p1-p3)** - Blessings of keeping commandments

</div>

<div class="p-2 bg-green-50 rounded-lg leading-tight">

**[John 14:15, 21](https://www.churchofjesuschrist.org/study/scriptures/nt/john/14?lang=eng&id=p15,p21)** - If ye love me, keep my commandments

</div>

<div class="p-2 bg-purple-50 rounded-lg leading-tight">

**[1 Nephi 3:7](https://www.churchofjesuschrist.org/study/scriptures/bofm/1-ne/3?lang=eng&id=p7)** - The Lord prepares a way

</div>

<div class="p-2 bg-yellow-50 rounded-lg leading-tight">

**[2 Nephi 31:10](https://www.churchofjesuschrist.org/study/scriptures/bofm/2-ne/31?lang=eng&id=p10)** - Follow Christ in obedience

</div>

<div class="p-2 bg-pink-50 rounded-lg leading-tight">

**[Mosiah 2:41](https://www.churchofjesuschrist.org/study/scriptures/bofm/mosiah/2?lang=eng&id=p41)** - Happy are they who keep commandments

</div>

<div class="p-2 bg-teal-50 rounded-lg leading-tight">

**[Alma 57:21](https://www.churchofjesuschrist.org/study/scriptures/bofm/alma/57?lang=eng&id=p21)** - Stripling warriors' exact obedience

</div>

<div class="p-2 bg-red-50 rounded-lg leading-tight" style="grid-column: span 2;">

**[D&C 130:20–21](https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/130?lang=eng&id=p20-p21)** - Blessings predicated on obedience to law

</div>

</div>

</div>

<!--
Presenter Notes:
- Give students 5-7 minutes to read and tag these scriptures
- They should look for principles about obedience
- Encourage them to tag their favorites as "obedience" in Gospel Library app
- They'll use these to create a talk outline next
- Move around the room to help as needed
-->

---
layout: default
---

# [John 14:15, 21](https://www.churchofjesuschrist.org/study/scriptures/nt/john/14?lang=eng&id=p15,p21)

<div class="space-y-4">

<div class="p-6 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

**Verse 15:** "If ye love me, keep my commandments."

**Verse 21:** "He that hath my commandments, and keepeth them, he it is that loveth me: and he that loveth me shall be loved of my Father, and I will love him, and will manifest myself to him."

</div>

</div>

<div class="p-4 bg-green-50 rounded-lg text-center">

<div class="text-base font-semibold">

Obedience is how we show our love for the Savior

</div>

</div>

</div>

<!--
Presenter Notes:
- Jesus taught that keeping commandments = loving Him
- It's not just about rules - it's about relationship
- When we obey, the Father and Son love us
- Christ will manifest Himself to us
- This applies directly to missionary work
-->

---
layout: default
---

# [1 Nephi 3:7](https://www.churchofjesuschrist.org/study/scriptures/bofm/1-ne/3?lang=eng&id=p7)

<div class="space-y-4">

<div class="p-6 bg-gradient-to-r from-green-50 to-teal-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"And it came to pass that I, Nephi, said unto my father: I will go and do the things which the Lord hath commanded, for I know that the Lord giveth no commandments unto the children of men, save he shall prepare a way for them that they may accomplish the thing which he commandeth them."

</div>

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center">

<div class="text-base font-semibold">

God prepares a way for us to keep His commandments

</div>

</div>

</div>

<!--
Presenter Notes:
- Nephi's attitude: "I will go and do"
- He trusted that God would prepare a way
- God never commands without enabling us to obey
- This is true for missionary work too
- If God calls us to serve, He'll prepare the way
-->

---
layout: default
---

# [Alma 57:21](https://www.churchofjesuschrist.org/study/scriptures/bofm/alma/57?lang=eng&id=p21)

<div class="space-y-4">

<div class="p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"Yea, and they did obey and observe to perform every word of command with exactness; yea, and even according to their faith it was done unto them; and I did remember the words which they said unto me that their mothers had taught them."

</div>

</div>

<div class="p-4 bg-blue-50 rounded-lg text-center">

<div class="text-base font-semibold">

The stripling warriors obeyed with exactness - and were blessed

</div>

</div>

</div>

<!--
Presenter Notes:
- The 2,000 stripling warriors were exact in their obedience
- They didn't pick and choose which commands to follow
- Their faith led to action
- Result: none of them died in battle
- Exact obedience brings divine protection and power
- This is a powerful example for missionaries
-->

---
layout: default
---

# [D&C 130:20–21](https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/130?lang=eng&id=p20-p21)

<div class="space-y-4">

<div class="p-6 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl">

<div class="text-base text-gray-800 leading-relaxed">

**20** "There is a law, irrevocably decreed in heaven before the foundations of this world, upon which all blessings are predicated—

**21** And when we obtain any blessing from God, it is by obedience to that law upon which it is predicated."

</div>

</div>

<div class="p-4 bg-green-50 rounded-lg text-center">

<div class="text-base font-semibold">

All blessings come through obedience to God's laws

</div>

</div>

</div>

<!--
Presenter Notes:
- This is a fundamental principle of the gospel
- Blessings are predicated on obedience
- This is an eternal law, decreed before the world was
- Want missionary blessings? Obey missionary standards
- Want the Spirit? Keep God's commandments
- This is cause and effect - it's how heaven works
-->

---
layout: default
---

# President Dieter F. Uchtdorf

<div class="space-y-4">

<div class="p-8 bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"Remember: the heavens will not be filled with those who never made mistakes but with those who recognized that they were off course and who corrected their ways to get back in the light of gospel truth."

</div>

<div class="text-sm text-gray-600 mt-4">

— President Dieter F. Uchtdorf<br/>
["A Matter of a Few Degrees,"](https://www.churchofjesuschrist.org/study/ensign/2008/05/a-matter-of-a-few-degrees?lang=eng) Ensign, May 2008

</div>

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center">

<div class="text-base font-semibold">

Obedience includes repenting and making course corrections when needed

</div>

</div>

</div>

<!--
Presenter Notes:
- Heaven is filled with people who made course corrections, not those who were perfect
- We all make mistakes - that's why repentance is part of obedience
- The key is recognizing when we're off course and correcting it
- Don't give up if you've made mistakes - repent and get back on track
- This is encouraging - obedience isn't perfection, it's continuing to turn to the Lord
- In missionary work, if we mess up, we repent and keep going
-->

---
layout: default
---

# Elder Ulisses Soares

<div class="space-y-4">

<div class="p-8 bg-gradient-to-r from-blue-50 to-teal-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"Obedience underlies our life decisions and allows God to open the heavens to shower His blessings upon us during our mortal existence and through eternity. Our obedience is a sign of our love, friendship and discipleship to the Savior who we represent as missionaries. Obedience to the commandments leads to blessings from God and shows our love for Him and for His Son Jesus Christ."

</div>

<div class="text-sm text-gray-600 mt-4">

— Elder Ulisses Soares, Quorum of the Twelve Apostles

</div>

</div>

</div>

<!--
Presenter Notes:
- Obedience opens the heavens
- It's a sign of our love for Christ
- It shows our discipleship
- We represent the Savior - obedience is essential
- This applies to all of us, not just full-time missionaries
-->

---
layout: default
---

# Elder Quentin L. Cook

<div class="space-y-4">

<div class="p-8 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">

<div class="text-lg text-gray-800 leading-relaxed">

"As you live righteously, you will not only escape bondage from the adversary, but you will also become an example of hope and light to others."

</div>

<div class="text-sm text-gray-600 mt-4">

— Elder Quentin L. Cook, Quorum of the Twelve Apostles<br/>
"Gathered in His Light: Avoiding Bondage," For the Strength of Youth, Oct. 2022

</div>

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center">

<div class="text-base font-semibold">

Our obedience makes us examples of hope and light

</div>

</div>

</div>

<!--
Presenter Notes:
- Living righteously frees us from Satan's bondage
- It also makes us examples to others
- We become "hope and light" when we're obedient
- This is how we bring others to Christ
- Our example is powerful missionary work
-->

---
layout: default
---

# How to Prepare a Talk

<div class="space-y-3">

## Build Your Talk with These Elements:

<div class="p-4 bg-blue-50 rounded-lg text-center font-semibold">

Topic

</div>

<div class="grid grid-cols-3 gap-3 text-sm">

<div class="p-4 bg-green-50 rounded-lg text-center">

**Scripture**

</div>

<div class="p-4 bg-purple-50 rounded-lg text-center">

**Gen Conf**

</div>

<div class="p-4 bg-yellow-50 rounded-lg text-center">

**Personal Experience**

</div>

</div>

<div class="p-4 bg-pink-50 rounded-lg text-center font-semibold mt-3">

Topic and Testimony

</div>

</div>

<!--
Presenter Notes:
- This is a simple framework for preparing talks
- Start with your TOPIC - what you're asked to speak about
- Find a SCRIPTURE that teaches about your topic
- Find a GEN CONF talk or quote from Church leaders
- Think of a PERSONAL EXPERIENCE that illustrates the principle
- End by restating your TOPIC and bearing your TESTIMONY
- Give students 5-7 minutes to create their outline using this framework
- They should write this in their journals or on paper
- Encourage them to use the scriptures they just studied
-->

---
layout: default
---

# Share Your Outline

<div class="space-y-4">

<div class="p-6 bg-blue-50 rounded-lg">

**Volunteer to share:** Would anyone like to share one part of your talk outline?

</div>

<div class="grid grid-cols-2 gap-4 text-sm">

<div class="p-4 bg-green-50 rounded-lg">

### You Could Share:

- Your chosen scripture
- An example you thought of
- A Church leader's quote
- Your personal testimony

</div>

<div class="p-4 bg-purple-50 rounded-lg">

### Remember:

- This is practice for real talks
- Speaking builds testimony
- Your words can inspire others
- The Spirit teaches through you

</div>

</div>

</div>

<!--
Presenter Notes:
- Invite 2-3 students to share briefly (2-3 minutes each)
- If no volunteers, ask specific students
- Affirm their responses and insights
- Point out good examples of applying scriptures
- Thank them for sharing
- Total time: 6-9 minutes
-->

---
layout: center
class: text-center
---

# Personal Application

<div class="space-y-6 max-w-3xl mx-auto">

<div class="p-8 bg-gradient-to-br from-yellow-50 to-pink-50 rounded-xl">

<div class="text-lg font-semibold mb-4">Think About:</div>

<div class="text-left space-y-2">

- One commandment you want to obey more fully

- How better obedience will help you share the gospel

- What you can do this week to show your love for Christ through obedience

</div>

<div class="text-sm italic text-gray-600 mt-4">Take a moment to ponder and write in your journal</div>

</div>

</div>

<!--
Presenter Notes:
- 2-3 minutes silent reflection
- Encourage journal writing
- This is personal application
- Students set their own goals
-->

---
layout: default
---

<div class="text-3xl font-bold mb-2">Key Takeaways</div>

<div class="text-xl font-semibold mb-3">Remember These Truths</div>

<div class="space-y-1 px-8">

<v-clicks>

<div class="p-1.5 bg-blue-50 rounded-lg text-xs leading-tight">

**Personal righteousness** qualifies us to represent the Savior and have the Spirit's companionship

</div>

<div class="p-1.5 bg-green-50 rounded-lg text-xs leading-tight">

**Obedience is how we show our love** for Heavenly Father and Jesus Christ ([John 14:15](https://www.churchofjesuschrist.org/study/scriptures/nt/john/14?lang=eng&id=p15))

</div>

<div class="p-1.5 bg-purple-50 rounded-lg text-xs leading-tight">

**God prepares a way** for us to keep His commandments ([1 Nephi 3:7](https://www.churchofjesuschrist.org/study/scriptures/bofm/1-ne/3?lang=eng&id=p7))

</div>

<div class="p-1.5 bg-yellow-50 rounded-lg text-xs leading-tight">

**All blessings are predicated on obedience** to God's laws ([D&C 130:20-21](https://www.churchofjesuschrist.org/study/scriptures/dc-testament/dc/130?lang=eng&id=p20-p21))

</div>

<div class="p-1.5 bg-pink-50 rounded-lg text-xs leading-tight">

**Without the Spirit, we cannot effectively share the gospel** - and obedience brings the Spirit

</div>

<div class="p-1.5 bg-teal-50 rounded-lg text-xs leading-tight">

**Our obedience makes us examples** of hope and light to others

</div>

</v-clicks>

</div>

<!--
Presenter Notes:
- These are the main points from today
- Review each one briefly
- Emphasize the connection between obedience, the Spirit, and missionary work
-->

---
layout: center
class: text-center
---

# Lesson Purpose Achieved

<div class="space-y-6 max-w-4xl mx-auto">

<div class="p-10 bg-gradient-to-r from-blue-50 to-teal-50 rounded-xl">

<div class="text-xl text-gray-800 leading-relaxed">

Obedience in doing the Savior's work—whether as a full-time missionary or as a member missionary—qualifies us to have the Spirit and effectively bring others to Christ.

</div>

<div class="text-lg text-blue-600 mt-6 italic">

Your obedience is a sign of your love for the Savior

</div>

</div>

</div>

<!--
Presenter Notes:
- Bring back lesson purpose
- Obedience is essential for missionary work
- This applies to all of us
- End with hope and encouragement
-->

---
layout: end
---

# Closing

<div class="text-center space-y-6">

## This Week's Challenge

<div class="max-w-xl mx-auto text-left space-y-3 bg-white p-6 rounded-lg text-base text-gray-900">

1. 📖 Review the scriptures on obedience and choose one to memorize

2. ✍️ Complete your talk outline on obedience in missionary work

3. 🎯 Set a specific obedience goal for this week

4. 💬 Share your testimony of obedience with someone

5. 🙏 Pray for help to be more obedient and to have the Spirit

</div>

<div class="mt-6 text-lg italic text-blue-600">

"If ye love me, keep my commandments." —John 14:15

</div>

## 🙏 Closing Prayer

</div>

<div class="abs-br m-6 text-xs text-gray-400">

Seminary Lesson - February 13, 2026

</div>

<!--
Presenter Notes:
- Give a clear challenge for the week
- Ask someone to say closing prayer
- Thank students for their participation
- Express testimony of the importance of obedience
- Testify that obedience brings the Spirit and power to share the gospel
-->
